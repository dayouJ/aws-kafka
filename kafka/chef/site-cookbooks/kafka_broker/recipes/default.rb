...
# Override kafka version if provided
if not ENV['kafka_version'].to_s.empty?
  node.override[:kafka][:version] = ENV['kafka_version']
  node.override[:kafka][:tarball_base] = "kafka_#{node[:kafka][:scala_version]}-#{node[:kafka][:version]}"
  node.override[:kafka][:tarball_name] = "#{node[:kafka][:tarball_base]}.tgz"
  node.override[:kafka][:tarball_url] = "https://archive.apache.org/dist/kafka/#{node[:kafka][:version]}/#{node[:kafka][:tarball_name]}"
end
